...
def union(pts_dir, img_dir):
    ...
    pts_key = copy.deepcopy(list(pts_dict['model_state'].keys()))
    img_key = copy.deepcopy(list(img_dict['state_dict'].keys()))
    #     pts_dict['model_state']["imgextraction."+key] = img_dict.pop(key)
    #for key in img_dict['state_dict']:
    for key in img_key:
        if key.split('.')[0] == 'backbone' or key.split('.')[0] == 'neck':
            pts_dict['model_state']["imgextraction."+key] = img_dict['state_dict'].pop(key)
        else:
            pts_dict['model_state']["dense_head_3d."+key] = img_dict['state_dict'].pop(key)
            
    torch.save(pts_dict, '%s_union_pgd.pth' % pts_dir[:-4])
