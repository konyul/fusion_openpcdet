...
from pcdet.utils.points import get_points_type


def apply_3d_transformation(pcd, coord_type, img_meta, sample_idx, reverse=False):
    ...
    pcd_rotate_mat = (
        torch.tensor(img_meta['pcd_rotation'][sample_idx][0], dtype=dtype, device=device)
        if 'pcd_rotation' in img_meta else torch.eye(
            3, dtype=dtype, device=device))

    pcd_scale_factor = (
        img_meta['pcd_scale_factor'][sample_idx] if 'pcd_scale_factor' in img_meta else 1.)

    pcd_trans_factor = (
        torch.tensor(img_meta['pcd_trans'][sample_idx], dtype=dtype, device=device)
    ...
    flow = img_meta['transformation_3d_flow'][sample_idx].tolist() \
