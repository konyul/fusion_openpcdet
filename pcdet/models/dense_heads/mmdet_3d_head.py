...
class MMDet3DHead(nn.Module):
    ...
    def get_loss(self, data_dict, tb_dict):
        ...
        img_metas = [{
            ...
            "img_shape": list(map(int,data_dict['img_shape'][0].cpu())),
            ...
            'img_norm_cfg': data_dict['img_norm_cfg'][i],
            'scale_factor': data_dict['scale_factor'][i].cpu().numpy() if 'scale_factor' in data_dict else None,
            'cam2img': data_dict['trans_cam_to_img'][0].cpu().tolist()+[[0,0,0,1]],
        ...
        gt_boxes_2d = data_dict['gt_boxes2d'][:,:,:-1]
        gt_labels_2d = data_dict['gt_boxes2d'][:,:,-1]
        ...
        data_dict['gt_boxes2d'] = torch.unbind(gt_boxes_2d)
